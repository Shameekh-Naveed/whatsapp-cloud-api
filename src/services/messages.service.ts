...
class MessagesService {
  ...
  async sendMessage(
    ...
    if (!response.ok) {
      const errorMessage = `Failed to send message to ${number}. Response: ${JSON.stringify(
        data
      )}`;
  ...
  private async handleConversationUpdate(
    ...
    try {
      ...
      const conversation =
        await this.conversationsService.findOrCreateConversation(number);
        ...
        for (const change of entry.changes) {
          ...
          if (value.messages) {
            ...
            for (const status of value.statuses) {
              ...
              if (statusType === "read") {
                await this.conversationsService.userSeenAllMessages(
                  conversation._id.toString()
                );
              }
            }
  ...
  private createRequestBody(
    ...
    } else if (template === MessageTemplates.NGO_REGISTERATION) {
      return {
        messaging_product: "whatsapp",
        to: number,
        type: "template",
  ...
  private async handleIncomingMessage(message: any, contacts: any[]) {
    ...
    const conversation =
      await this.conversationsService.findOrCreateConversation(phoneNumber);
